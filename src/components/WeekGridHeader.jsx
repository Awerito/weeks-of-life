      ...
      <div className="md:hidden sticky top-2 z-10 -mx-6 -mt-6 mb-2">
        <div className="bg-white dark:bg-gray-800 px-6 pt-6 pb-2 rounded-t-lg shadow-[0_-16px_0_0_#f9fafb] dark:shadow-[0_-16px_0_0_#111827]">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
            {t("grid.lifeExpectancy")}{" "}
            <span className="font-medium">
              {stats.lifeExpectancyYears.toFixed(1)} {t("grid.years")}
            </span>{" "}
            ({sexLabel})
          </p>
          <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-gray-400 mb-2">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-[#3B82F6] rounded-sm"></span> {t("legend.lived")}
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-[#10B981] rounded-sm"></span> {t("legend.now")}
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-[#EC4899] rounded-sm"></span> {t("legend.midpoint")}
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 bg-gray-200 rounded-sm"></span> {t("legend.future")}
            </span>
            {stats.extraWeeks > 0 && (
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 bg-[#F5D02F] rounded-sm"></span> {t("legend.bonus")}
              </span>
            )}
          </div>
